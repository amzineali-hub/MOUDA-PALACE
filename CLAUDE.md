...
- **RH** : suivi des absences (jour complet et heures partielles) avec déduction salariale au prorata, correction d'un bulletin de paie généré avec un nom d'employé vide, nouvel onglet Documents RH (congés, modèles légaux téléchargeables), pointage manuel, avance/carte sanitaire, suppression dans l'historique de paie. Puis : ajout des onglets Évaluations, Formations et Rôles & Accès (jusque-là codés mais invisibles, sans aucune table d'affichage) ; Formations bascule du state React local vers Firestore ; retrait du planning dupliqué mort (`scheduleData`/`isShiftModalOpen`, le vrai planning est `PlanningScheduler.tsx`) ; bulletin de paie imprimé désormais rempli avec les vraies infos employé (poste, N°CNSS, date de naissance, situation familiale, date d'embauche, période de paie) au lieu de valeurs figées — ajout de deux champs employé (date de naissance, situation familiale) et dénormalisation de ces données dans la fiche de paie au moment de la génération pour préserver l'historique si l'employé est modifié/supprimé plus tard. Puis : ajout de la génération de Contrats et avenants (CDI, CDD, Convention de stage) dans l'onglet Documents RH, texte des articles repris des modèles Word existants (`templates/rh/Contrat_CDI_Modele.docx`, `Contrat_CDD_Modele.docx`, extraits via `mammoth`) et rempli avec les vraies données employé ; correction au passage d'un bug de format déjà présent sur Attestation/Certificat (les dates d'embauche/fin s'affichaient au format ISO brut au lieu de jj/mm/aaaa).
- **Comptabilité** : numérotation séquentielle des factures, en-tête/logo réels de l'entreprise, en-tête de facture unifié, modèles de menu de groupe éditables, détail HT/TVA/TTC dans Dépenses & Achats, édition/suppression de factures. Puis, suite à un audit du 2026-08-20 : correction d'un double comptage des achats fournisseurs (chaque commande livrée était comptée deux fois — une entrée synthétique dérivée de `commandes` en plus de la vraie dépense créée à la réception — et avec le mauvais montant en prime) en supprimant la dérivation synthétique désormais redondante (`Accounting.tsx` `expenses` = uniquement la collection Firestore `expenses`) ; TVA (`montantHT`/`tva`) désormais reportée sur la dépense réelle créée à la réception d'une commande (`AchatsFournisseurs.tsx` `ReceptionAchats.handleValidate`), qui était jusque-là ignorée par la Déclaration TVA ; retrait du faux "Bilan Comptable" (chiffres entièrement inventés mais crédibles, Actif=Passif équilibrés) au profit du même message honnête "en cours de développement" que les autres rapports non implémentés ; export CSV des dépenses/factures sécurisé contre un montant non-chaîne (`parseAmount` au lieu de `.replace` direct).
...
- **POS/Caisse tactile** : élargissement du panneau ticket, boutons pleins 3D (ombre en relief, s'enfonce au clic) sur les onglets du ticket (Ticket Actuel/Cuisine KDS/En attente/Messages) et sur Rembourser/Rapport X. Bug corrigé : le Rapport X/Z de caisse imprimait une feuille blanche — le CSS d'impression (`:has()` + portail caché) ne fonctionnait pas de façon fiable sur imprimante thermique réelle ; remplacé par le même mécanisme que factures/documents RH (fenêtre dédiée autonome + impression auto), qui lui fonctionne. Même style 3D étendu à l'Écran Cuisine (KDS) : "Vider les démonstrations", filtres de station, boutons Commencer/Terminer.
- **RH — conformité Code du travail marocain** : audit des documents générés (CDI, CDD, stage, attestation, certificat, solde de tout compte) et du calcul de paie. Corrigé : barème IGR mensuel mis à jour vers le barème réformé (LF2023, en vigueur 2026 — tranches 3 333/5 000/6 667/8 333/15 000 MAD, taux 10 à 37%, contre l'ancien barème 2 500/4 166/5 000/6 666/15 000 à 10-38% qui sur-retenait l'impôt) ; frais professionnels passés à 25% plafonné 2 916,66 MAD/mois (au lieu de 20%/2 500) ; ajout de l'abattement "charges de famille" (5 MAD/mois/personne à charge, max 6) dans `computePayroll` — inactif tant que la fiche salarié ne stocke pas ce nombre (voir `BACKLOG.md`). CDD : motif limité aux 3 cas légaux de l'article 16 (liste déroulante), alerte automatique si durée > 1 an (risque de requalification en CDI), période d'essai calculée et affichée (article 15). CDI : ajout d'une clause de période d'essai selon catégorie (cadre/employé/ouvrier, article 14). Solde de tout compte : ajout de la mention légale du droit de dénonciation à 60 jours (article 73).
