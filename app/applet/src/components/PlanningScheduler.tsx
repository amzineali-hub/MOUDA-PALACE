...
import { ChevronLeft, ChevronRight, Plus, Sparkles, Filter, MoreHorizontal, User as UserIcon } from 'lucide-react';
...
export default function PlanningScheduler({ staffData }: { staffData: any[] }) {
  ...
  const addNewShift = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Simple hour calculation (assuming same day)
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col min-h-[600px]">
      ...
      <div className="p-4 border-b border-gray-200 flex flex-wrap justify-between items-center bg-white gap-4">
        ...
        <div className="flex items-center gap-2">
          <button className="px-4 py-2 bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-lg text-sm font-medium hover:bg-emerald-100 flex items-center gap-2 transition-colors">
            <Sparkles size={16} /> IA Planning
          </button>
          <button className="px-4 py-2 bg-[#F4C75B] text-[#1A1A1A] rounded-lg text-sm font-medium hover:bg-[#E5B745] flex items-center gap-2 transition-colors">
