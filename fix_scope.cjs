...
const aggCode = `  const revByDate = cashReceipts.reduce((acc, cr) => {
...
code = code.replace(aggCode, "");

const insertPoint = `  const recipeChartData = recipes.slice(0, 5).map(r => ({`;
code = code.replace(insertPoint, aggCode + "\\n\\n" + insertPoint);
